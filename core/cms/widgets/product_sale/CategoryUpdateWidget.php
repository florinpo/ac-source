...
class CategoryUpdateWidget extends CWidget {
    ...
    protected function renderContent() {
        ...
        $model = GxcHelpers::loadDetailModel('ProductSaleCategoryList', $cat_id);
        ...
        if (isset($_POST['ProductSaleCategoryList'])) {
            ...
            $model->name = trim($_POST['ProductSaleCategoryList']['name']);
            $model->description = trim($_POST['ProductSaleCategoryList']['description']);
            ...
            if ($model->validate()) {
                ...
                if ($new_order_id !== $model->id && $old_parent_id != 0) {
                    $old_parent = GxcHelpers::loadDetailModel('ProductSaleCategoryList', $old_parent_id);
                    $children = $old_parent->children()->findAll();

                    $first = reset($children);
                    $first_node = GxcHelpers::loadDetailModel('ProductSaleCategoryList', $first->id);
                    $last = end($children);
                    $last_node = GxcHelpers::loadDetailModel('ProductSaleCategoryList', $last->id);


                    //the previous sibling node (after the move).
                    if ($new_order_id != '-1' && $new_order_id != '-2')
                        $new_order_node = GxcHelpers::loadDetailModel('ProductSaleCategoryList', $new_order_id);
                ...
                if ($new_parent_id != 'empty') {
                    ...
                    $new_parent = GxcHelpers::loadDetailModel('ProductSaleCategoryList', $new_parent_id);
                    //the previous sibling node (after the move).

                    if ($previous_node_id != '0')
                        $previous_node = GxcHelpers::loadDetailModel('ProductSaleCategoryList', $previous_node_id);
        ...
        $this->render('cmswidgets.views.product_sale.category_form_widget', array('model' => $model));
    }
