        ...
        if ($cache_id) {
            switch ($cache_id) {
                case 'frontend_assets':
                    ...
                    user()->setFlash('success', Yii::t('AdminCache', 'FRONTEND Assets cleared!'));
                    break;
                case 'frontend_cache':
                    if ($this->clearCache('frontend'))
                        user()->setFlash('success', Yii::t('AdminCache', 'FRONTEND Cache cleared!'));
                    else
                        user()->setFlash('error', Yii::t('AdminCache', 'Error while clear FRONTEND Cache!'));
                    break;
                case 'backend_assets':
                    $this->clearCacheAssets('backend');
                    user()->setFlash('success', Yii::t('AdminCache', 'BACKEND Assets cleared!'));
                    break;
                case 'backend_cache':
                    if ($this->clearCache('backend'))
                        user()->setFlash('success', Yii::t('AdminCache', 'BACKEND Cache cleared!'));
                    else
                        user()->setFlash('error', Yii::t('AdminCache', 'Error while clear BACKEND Cache!'));
                    break;
                default:
                    break;
            }
            Yii::app()->controller->redirect(array('caching/cachemanagement'));
        }
    ...
    public function clearCache($where) {
        switch ($where) {
            case 'frontend':
                ...
                curl_setopt($curl, CURLOPT_URL, FRONT_SITE_URL . '/site/cachingclear');
