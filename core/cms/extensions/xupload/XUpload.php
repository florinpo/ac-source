...
class XUpload extends CJuiInputWidget {
    ...
    /**
     * @var array to keep array options in form view
     */
    public $formOptions = array();
    
    
    /**
     * @var integer to keep the parent id
     */
    ...
    public function run() {
        ...
        //$htmlOptions = array();
        if ($this -> multiple) {
            $this->formOptions["multiple"] = true;
            /* if($this->hasModel()){
        ...
        if (is_dir($assets)) {
            ...
            $messages = CJavaScript::encode(array(
                'fileupload' => array(
                    ...
                    'cancellogo' => $this->t('Cancela logo'),
                    'cancelavatar' => $this->t('Cancela imagine'),
                ),
            ));
