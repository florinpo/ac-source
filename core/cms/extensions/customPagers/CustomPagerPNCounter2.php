...
class CustomPagerPNCounter extends CLinkPager {
    ...
    public function run() {
        ...
        if (empty($buttons))
            return;
        echo $this->header;
        $list = CHtml::tag('ul', array('class'=>'btn-group toolbar'), implode("\n", $buttons));
    ...
    protected function createPageButton($label, $page, $class, $hidden, $selected) {
        if ($hidden || $selected)
         ...
         $link = CHtml::link($label, $this->createPageUrl($page), array('class' => 'buttonM bDefault wLb'));
         return CHtml::tag('li', array('class' => $class), $link);
        //return CHtml::link($link, $this->createPageUrl($page), array('class' => $class));
    }
    ...
    protected function createPageDisplay($current_page, $total) {
        if($this->counter == true)
        $content = CHtml::openTag('span', array()) . $current_page . '/' . $total . CHtml::closeTag('span');
        return CHtml::tag('li', array('class' => 'page-display'), $content);
