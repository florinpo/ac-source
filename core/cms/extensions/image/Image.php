...
Yii::import('cms.extensions.image.Image_Driver');
...
class Image {
    ...
    public function __construct($image, $config = NULL) {
        ...
        // Load the driver
        Yii::import("cms.extensions.image.drivers.$driver");
    ...
    public function square($background = NULL) {
        $background = ltrim($background, '#');

        if (!preg_match('/^[a-f0-9]{6}$/i', $background)) { // if hex color not valid, use white
            $background = 'FFFFFF';
        }

        $this->actions['square'] = array('background' => $background);

        return $this;
    }

    public function square_fixed($width, $height, $background = NULL) {
        $background = ltrim($background, '#');
        ...
        $this->actions['square_fixed'] = array(
            'width' => $width,
            'height' => $height,
