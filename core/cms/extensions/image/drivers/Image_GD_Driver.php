...
class Image_GD_Driver extends Image_Driver {
    ...
    public function square($properties) {
        $img = $this->tmp_image;

        $width = imagesx($img);
        $height = imagesy($img);

        list($properties['red'], $properties['green'], $properties['blue']) = str_split($properties['background'], 2);

        $square_size = max($width, $height);

        $blankbox = imagecreatetruecolor($square_size, $square_size);
        $background_color = imagecolorallocate($blankbox, 255, 255, 255);
        imagefill($blankbox, 0, 0, hexdec($properties['background']));

        $offset_x = floor(($square_size - $width) / 2);
        $offset_y = floor(($square_size - $height) / 2);

        imagecopyresampled($blankbox, $img, $offset_x, $offset_y, 0, 0, $width, $height, $width, $height);

        $this->tmp_image = $blankbox;

        return $blankbox;
    }

    public function square_fixed($properties) {
        $img = $this->tmp_image;
        ...
        $hratio = $h / imagesy($img);
        $wratio = $w / imagesx($img);
        ...
        $background_color = imagecolorallocate($thumbnail, 255, 255, 255);
        imagefill($thumbnail, 0, 0, hexdec($properties['background']));


        imagecopyresampled($thumbnail, $img, $dst_x, $dst_y, 0, 0, $dst_w, $dst_h, $orig_w, $orig_h);

        $this->tmp_image = $thumbnail;
