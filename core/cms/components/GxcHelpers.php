...
class GxcHelpers {
    ...
    public static function getContentType($first = true) {
        $types = ContentType::getTypes();
        if ($first) {
            $result = array('0' => t('cms', '-Select type-'));
        } else {
            $result = array();
        }
    ...
    // return display name
    public static function getDisplayName($userId, $strip = false, $inc_uname = false) {
        $user = User::model()->findByPk($userId);
        if ($user) {

            if ($inc_uname == true) {
                $display_name = "<span class='usrcls'>" . $user->display_name . " (" . $user->username . ")" . "</span>";
            } else
                $display_name = "<span class='usrcls'>" . $user->display_name . "</span>";
        } else {
            $user = Yii::app()->db->createCommand()
                    ->select('display_name, username')
                    ->from('gxc_users_klist')
                    ->where('user_id=:id', array(':id' => $userId))
                    ->queryRow();
            if ($user) {
                if ($inc_uname == true)
                    $display_name = "<span class='line-through usrcls'>" . $user['display_name'] . " (" . $user['username'] . ")" . "</span>";
                else
                    $display_name = "<span class='line-through usrcls'>" . $user['display_name'] . "</span>";
            } else
                return false;
        }
        if ($strip == false)
            ...
            return strip_tags($display_name);
    }

    /**
     * return an array of months short format
     */
    public static function monthNamesShort() {
        $months = array();
        $months[0] = t('cms', 'Jan');
        $months[1] = t('cms', 'Feb');
        $months[2] = t('cms', 'Mar');
