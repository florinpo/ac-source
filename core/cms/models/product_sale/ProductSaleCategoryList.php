...
class ProductSaleCategoryList extends CActiveRecord {
    ...
    public function tableName() {
        return '{{product_sale_category_list}}';
    }
...
//            ),
            ...
            array('product_id, category_id, description, parent_id, order_id, guid', 'safe'),
    ...
    public function relations() {
        ...
        return array(
            'products' => array(self::MANY_MANY, 'ProductSale', 'gxc_product_sale_category(category_id, product_id)'),
            'productsCount' => array(self::STAT, 'ProductSale', 'gxc_product_sale_category(category_id, product_id)'),
    ...
    protected function afterDelete() {
        ...
        if ($this->productsCount > 0) {
            ProductSaleCategory::model()->deleteAll('category_id=' . $this->id);
        }
    }
    ...
    public static function getCategories($id, $render = true){
        
         $parent = ProductSaleCategoryList::model()->findByPk($id);
         $categories = $parent->children()->findAll();
         //$data = array('empty' => ''));
         $data = array();
         
         if($categories && count($categories)>0) {
              $data = CMap::mergeArray($data, CHtml::listData($categories, 'id', 'name'));
         }
